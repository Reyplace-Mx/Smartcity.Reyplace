...
export default function ThreeDMap({
    ...
    places.forEach((place, index) => {
      ...
      if (isDigital) {
        ...
        if (isClinic) {
          ...
          // Glass Roof Balcony
          const balcG = new THREE.BoxGeometry(width * 0.8, 0.5, depth * 0.8);
          const balcM = new THREE.MeshBasicMaterial({ color: 0x10b981, transparent: true, opacity: 0.8 });
          const balc = new THREE.Mesh(balcG, balcM);
          balc.position.y = height / 2 + 0.3;
          building.add(balc);
        }
  ...
  return (
    <div className="relative w-full h-full min-h-[550px] overflow-hidden rounded-3xl border border-slate-800 bg-slate-950 shadow-2xl">
      ...
      {/* Auto Tour 3D Floating Control Bar & Nanite / RTX Bar */}
